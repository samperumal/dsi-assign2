...
A pre-requiste of topic modelling is understanding the number of topics (latent factor k) that each corpus may contain.  In some case, this may be a fair assumption but as in our case, how would we, without reading though each speech, know how many different topics would be discussed?  Luckily, Murzintcev Nikita has published a R- package that helps to optimise the number of topics(k) over four different measure.  The four measures are discussed in an RPubs paper which can found here: http://www.rpubs.com/MNidhi/NumberoftopicsLDA:
