...
predictions_2 <- model_attempt %>% predict_classes(val_x)
...
predictions_val_2 <- cbind(sentence_data$validate,predictions_2,pres.actual)
View(head(predictions_val_2, n= 100))
dim(validate$x)
##I can't connect back to sentecne as the tokeniser drops 4 lines?


predictions_val_2$predictions_2 <-  predictions_val_2$predictions_2 + 1
max(predictions_val_2$predictions_2)
min(predictions_val_2$predictions_2)


max(pres.actual)
min(pres.actual)

sqrt(mean(sum((predictions_val_2$predictions_2 - predictions_val_2$pres.actual)^2)))

t <- table(predictions_val_2$pres.actual, predictions_val_2$predictions_2)
sum(diag(t))/sum(t)
