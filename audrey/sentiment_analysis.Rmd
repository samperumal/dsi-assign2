...
It just so happens that the 4 negation words are also stop words so have already been removed from our Bigrams. This can be shown as follows:
...
So we need to first deal with negation and then remove stop words:

```{r negation}

# start with the bigrams including stop words
bigrams_ex_negation <- tidy_sona_sentences %>% 
  unnest_tokens(bigram, sentence, token = "ngrams", n = 2) %>% 
  # separate the bigrams
  separate(bigram, c("word1", "word2"), sep = " ") %>% 
...
filter(bigrams_ex_negation, word1 %in% negation_words) %>% 
  select(year, word1, word2, sentiment1, sentiment2) %>% 
  filter(sentiment2 != "neutral") %>%    # we are not interested in these
  count()
  
```

There are 118 bigrams that contain negation words. We need to reverse the sentiment of word 2.
