...
knitr::opts_chunk$set(
  echo = FALSE, eval = TRUE, warning = FALSE, fig.align = "center"
)
...
The CNN did not provide an improvement over the initial NN, and as a result we tried a Recurrent Neural Net (RNN). This takes in a sequence of data and gives importance to the order of the words in order to make a prediction.
...
avg_word_count <- tidy_sona_words %>%
  ...
  left_join(speech_count, by = "president") %>% 
...
word_count %>%
  ...
  geom_point() + 
  geom_smooth(method = "loess", aes(colour = president)) + xlab("Year") + ylab("Number of words")
...
```
![Figure: Common bigram wordcloud](WordCloud.png)
...
tidy_sona_words %>%
  ...
  group_by(president) %>%
  count(word, sort = TRUE) %>%
  left_join(total_speeches, by = "president") %>% 
  mutate(freq = n/total) %>% 
...
ggplot(sentiments_per_year, aes(x = president, y = avg_sentiment, colour = president)) + 
  ...
  ggtitle("Boxplots of bing Sentiment per President") + xlab("President") + ylab("Average Sentiment")
...
sentiments_per_year %>%
  ...
  scale_x_discrete(name ="year", 
                    limits=c(1994,1999,2004,2009,2014)) + xlab("President") + ylab("Average Sentiment")
...
ggplot(sentiments_per_year, aes(x = president, y = afinn_sentiment_perc, colour = president)) +   geom_boxplot() + # draw a boxplot for each president
  ggtitle("Boxplots of afinn sentiment per President") + xlab("President") + ylab("Afinn Sentiment %")
...
sentiments_per_year %>%
  ...
  scale_x_discrete(name ="year", 
                    limits=c(1994,1999,2004,2009,2014)) + xlab("President") + ylab("Afinn Sentiment")
...
tidy_sona_words %>%
  ...
  left_join(president_total, by = "president") %>% 
...
filter(bigrams_negation, word1 %in% negation_words) %>% 
  ...
  group_by(president) %>% 
  count() %>% 
  left_join(president_totalbigrams, by = "president") %>% 
...
ggplot(aes(bigram, count_bigram)) +
  ...
  coord_flip()  + xlab("Count") + ylab("Bigram")
...
From the above plot, the marginal benefit from adding another topic, stops at around 5-10 topics.  In order to test this, the *"perplexity"* over a test sample for the document term matrix can be checked.
...
```
![Figure: Comparison of biggest differential between Topic 1 and Topic 5 Bigram](images/topiccompare.png)
...
kable(sample, caption = "Sample sentence showing topic probabilities") %>% kable_styling()
...
```
![Figure: Probability of belonging to each Topic, by President](images/pres_topic.png)
...
![Figure: Change in topic composition over time, per president](images/topic_time.png)
...
load("nnm2.RData")
```
![Figure: Bag of words model - neural net training performance](images/test_accuracy_wordcount_bigger.png)
...
```
![Figure: Bag of words smaller model - neural net training performance](images/test_accuracy_wordcount_smaller.png)

`r table_bow %>% kable() %>% kable_styling()`
...
```
```{r}
table_tfidf %>% kable() %>% kable_styling()
```
![Figure: TF-IDF model - neural net training performance](images/test_accuracy_tfidf.png)
...
```
```{r}
table_sentiment = cbind(table_sentiment, "4" = rep(0, 6), "5" = rep(0, 6),"6" = rep(0, 6))
table_sentiment %>% kable() %>% kable_styling()
```

![Figure: Sentiment Analysis - neural net training performance](images/test_accuracy_sentiment.png)
...
Cohen's Kappa: `r cohen.kappa(table_sentiment)[1]`
...
![Figure: Topic Modelling - neural net training performance](images/test_accuracy_topic.png)



```{r eval=FALSE}

table_topic = cbind(table_topic, "3"=rep(0,6), "3"=rep(0,6), "4" = rep(0, 6), "5" = rep(0, 6),"6" = rep(0, 6))#make it square
#cohen.kappa(table_topic)[1] #it is doing fairly better than a randomly guessing classifier.
...
Cohen's Kappa: `r cohen.kappa(table_topic)[1]`
...
![Figure: Glove transfer learning - neural net training performance](images/test_accuracy_transferlearning.png)
...
With a Kappa value of `r cohen.kappa(t)[1]`, we are doing moderately better than a random classifier.
...
```{r nn, eval=TRUE}
result_data = rbind(
  c("Bigger word-count", train_accuracy_wordcount_bigger$acc, test_accuracy_wordcount_bigger$acc),
  c("Smaller word-count", train_accuracy_wordcount_smaller$acc, test_accuracy_wordcount_smaller$acc),
  c("TF-IDF model", train_accuracy_tfidf$acc, test_accuracy_tfidf$acc),
  c("Sentiment Analysis", train_accuracy_sentiment, test_accuracy_sentiment),
  c("Topic modelling", test_accuracy_topic$acc, train_accuracy_topic$acc),
  c("Transfer Learning", train_accuracy_transferlearn$acc, test_accuracy_transferlearn$acc),
  c("CNN", 0.9946, 0.6109),
  c("RNN", 0.35, 0.35)
)

colnames(result_data) = c("Model", "Train Accuracy", "Test Accuracy")

result_data[,2] = round(as.numeric(result_data[,2]), 2)
result_data[,3] = round(as.numeric(result_data[,3]), 2)

result_data %>% kable() %>% kable_styling()
```
