...
model %>%
  ...
  layer_dense(units = 16, activation = 'relu', 
              kernel_regularizer = regularizer_l2(0.01)) %>%  #Dense layer with L2 regulerizer 
  layer_dropout(0.01) %>%
...
```
Sentiment analysis also reaches it's smallest validation loss value on the second iteration. But the train accuracy and test accuracy is changing very slightly at each iteration. This model does not seem to be doing well on eighter the training set or the test set.
...
topic_matrix %>% left_join(sentiment_all)
```
