...
```{r fig.cap="Change in number of sentences per president before and after filtering.", eval=TRUE}
...
table_bow <- table(predictions_val$pres.actual, predictions_val$pres.max)
...
#Calculate kappa using a cohen.kappa()

table_bow = cbind(table_bow, "4" = rep(0, 6), "5" = rep(0, 6),"6" = rep(0, 6))#make it square
#As we have seen from the table above the nn did not predict anything for the minority classes so we have to cbind zeros as 4th,5th,6th columns. Because we need a square matrix for calculating Cohen's Kappa when using cohen.kappa() function.
cohen.kappa(table_bow)[1]
