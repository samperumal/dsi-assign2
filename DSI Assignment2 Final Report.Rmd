...
#Evaluate the model
model %>% evaluate(x_test, y_test, batch_size=32, verbose = 1) #0.55886
```
...
model %>% evaluate(x_test, y_test, batch_size=32, verbose = 1) #0.6146221 accuracy rate
...
###Neural Nets with Sentiment Analysis Data
```{r nnwith}
...
(accuracy_sentiment_test = (model %>% evaluate(x_test, y_test))$acc)
(accuracy_sentiment_train = (model %>% evaluate(x_train, y_train))$acc)
...
### Neural Nets with Topic Modelling Gamma Values
```{r}
...
predictions_val %>% filter(president == "Mbeki") %>% select(pres.max, pres.actual) #President numero 1
predictions_val %>% filter(president == "Zuma") %>% select(pres.max, pres.actual) #President numero 2
predictions_val %>% filter(president == "Mandela") %>% select(pres.max, pres.actual) #President numero 3
predictions_val %>% filter(president == "Motlanthe") %>% select(pres.max, pres.actual) #President numero 4
predictions_val %>% filter(president == "Ramaphosa") %>% select(pres.max, pres.actual) #President numero 5
predictions_val %>% filter(president == "deKlerk") %>% select(pres.max, pres.actual) #President numero 6
