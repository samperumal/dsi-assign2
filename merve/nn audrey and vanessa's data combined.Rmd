...
topic_matrix = as.tibble(topic_matrix)
sentiment_topic_combined = sentiment_all %>% inner_join(topic_matrix)
sentiment_topic_combined[1,]
```
...
x_train <- as.matrix(training_sona %>% select(-presidents, -filename, -text))
x_test <- as.matrix(test_sona %>% select(-presidents, -filename, -text))
