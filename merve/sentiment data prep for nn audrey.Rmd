...
```{r}
sentiment_all$presidents = gsub(".*[_]([^.]+)[.].*", "\\1", (sentiment_all$filename))
```
...
x_train <- as.matrix(training_sona %>% select(-presidents, -filename))
x_test <- as.matrix(test_sona %>% select(-presidents, -filename))
...
test_sona %>% select(presidents) %>% unique
```
